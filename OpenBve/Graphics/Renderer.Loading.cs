...
namespace OpenBve {
	internal static partial class Renderer 
	{
		internal static void DrawLoadingScreen() {
			...
			DrawRectangle(null, new Point(0, 0), new Size(Screen.Width, Screen.Height), RGBAf.Black);
			if (Textures.LoadTexture(TextureLogo, Textures.OpenGlTextureWrapMode.ClampClamp)) {
				DrawRectangle(TextureLogo, new Point((Screen.Width - size) / 2, (Screen.Height - size) / 2), new Size(size, size), RGBAf.White);
			}
			DrawRectangle(null, new Point((Screen.Width - size) / 2, Screen.Height - (int)Fonts.NormalFont.FontSize - 10), new Size(Screen.Width, (int)Fonts.NormalFont.FontSize + 10), new RGBAf(0.0f, 0.0f, 0.0f, 0.5f));
...
//			}
			...
			DrawString(Fonts.SmallFont, text, new Point((Screen.Width - size) / 2 + 5, Screen.Height - (int)(Fonts.NormalFont.FontSize / 2) - 5), TextAlignment.CenterLeft, RGBAf.White);
			// end HACK //
		}
	}
}
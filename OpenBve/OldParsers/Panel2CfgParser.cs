...
namespace OpenBve 
{
	internal static class Panel2CfgParser 
	{
		...
		internal static void ParsePanel2Config(string TrainPath, System.Text.Encoding Encoding, TrainManager.Train Train) {
			...
			string FileName = Common.Path.CombineFile(TrainPath, "panel2.cfg");
			...
			RGBb PanelTransparentColor = new RGBb(0, 0, 255);
										...
										switch (Key.ToLowerInvariant()) {
											...
											case "daytimeimage":
												...
												if (Interface.ContainsInvalidPathChars(Value)) {
													...
													PanelDaytimeImage = Common.Path.CombineFile(TrainPath, Value);
											...
											case "nighttimeimage":
												...
												if (Interface.ContainsInvalidPathChars(Value)) {
													...
													PanelNighttimeImage = Common.Path.CombineFile(TrainPath, Value);
			...
			if (PanelDaytimeImage != null) {
				if (!System.IO.File.Exists(PanelDaytimeImage)) {
					...
					Textures.RegisterTexture(PanelDaytimeImage, new OpenBveApi.Textures.TextureParameters(null, new RGBb(PanelTransparentColor.R, PanelTransparentColor.G, PanelTransparentColor.B)), out tday);
					...
					if (PanelNighttimeImage != null) {
						if (!System.IO.File.Exists(PanelNighttimeImage)) {
							...
							Textures.RegisterTexture(PanelNighttimeImage, new OpenBveApi.Textures.TextureParameters(null, new RGBb(PanelTransparentColor.R, PanelTransparentColor.G, PanelTransparentColor.B)), out tnight);
						}
					}
					...
					CreateElement(Train, 0.0, 0.0, PanelBitmapWidth, PanelBitmapHeight, 0.5, 0.5, 0.0, PanelResolution, PanelLeft, PanelRight, PanelTop, PanelBottom, PanelBitmapWidth, PanelBitmapHeight, PanelCenterX, PanelCenterY, PanelOriginX, PanelOriginY, DriverX, DriverY, DriverZ, tday, tnight, new RGBAb(255, 255, 255, 255), false);
				}
			}
							...
							case "pilotlamp":
								{
									...
									RGBb TransparentColor = new RGBb(0, 0, 255);
									int Layer = 0;
									i++; while (i < Lines.Length && !(Lines[i].StartsWith("[", StringComparison.Ordinal) & Lines[i].EndsWith("]", StringComparison.Ordinal))) {
										int j = Lines[i].IndexOf('=');
										if (j >= 0) {
											...
											switch (Key.ToLowerInvariant()) {
												...
												case "daytimeimage":
													...
													if (Interface.ContainsInvalidPathChars(Value)) {
														...
														DaytimeImage = Common.Path.CombineFile(TrainPath, Value);
												...
												case "nighttimeimage":
													...
													if (Interface.ContainsInvalidPathChars(Value)) {
														...
														NighttimeImage = Common.Path.CombineFile(TrainPath, Value);
									...
									if (DaytimeImage != null) {
										Textures.Texture tday;
										Textures.RegisterTexture(DaytimeImage, new OpenBveApi.Textures.TextureParameters(null, new RGBb(TransparentColor.R, TransparentColor.G, TransparentColor.B)), out tday);
										Textures.Texture tnight = null;
										if (NighttimeImage != null) {
											Textures.RegisterTexture(NighttimeImage, new OpenBveApi.Textures.TextureParameters(null, new RGBb(TransparentColor.R, TransparentColor.G, TransparentColor.B)), out tnight);
										}
										Textures.LoadTexture(tday, Textures.OpenGlTextureWrapMode.ClampClamp);
										int w = tday.Width;
										int h = tday.Height;
										int j = CreateElement(Train, LocationX, LocationY, w, h, 0.5, 0.5, (double)Layer * StackDistance, PanelResolution, PanelLeft, PanelRight, PanelTop, PanelBottom, PanelBitmapWidth, PanelBitmapHeight, PanelCenterX, PanelCenterY, PanelOriginX, PanelOriginY, DriverX, DriverY, DriverZ, tday, tnight, new RGBAb(255, 255, 255, 255), false);
							...
							case "needle":
								{
									...
									double LocationX = 0.0, LocationY = 0.0;
									string DaytimeImage = null, NighttimeImage = null;
									RGBAb Color = new RGBAb(255, 255, 255, 255);
									RGBb TransparentColor = new RGBb(0, 0, 255);
									...
									if (DaytimeImage != null & Interval > 0) {
										...
										if (wday > 0 & hday > 0) {
											...
											for (int k = 0; k < nday; k++) {
												Textures.RegisterTexture(DaytimeImage, new OpenBveApi.Textures.TextureParameters(new OpenBveApi.Textures.TextureClipRegion(0, k * Interval, wday, Interval), new RGBb(TransparentColor.R, TransparentColor.G, TransparentColor.B)), out tday[k]);
											}
											if (NighttimeImage != null) {
												int wnight, hnight;
												...
												for (int k = 0; k < nnight; k++) {
													Textures.RegisterTexture(NighttimeImage, new OpenBveApi.Textures.TextureParameters(new OpenBveApi.Textures.TextureClipRegion(0, k * Interval, wday, Interval), new RGBb(TransparentColor.R, TransparentColor.G, TransparentColor.B)), out tnight[k]);
												}
											...
											for (int k = 0; k < tday.Length; k++) {
												int l = CreateElement(Train, LocationX, LocationY, (double)wday, (double)Interval, 0.5, 0.5, (double)Layer * StackDistance, PanelResolution, PanelLeft, PanelRight, PanelTop, PanelBottom, PanelBitmapWidth, PanelBitmapHeight, PanelCenterX, PanelCenterY, PanelOriginX, PanelOriginY, DriverX, DriverY, DriverZ, tday[k], tnight[k], new RGBAb(255, 255, 255, 255), k != 0);
												if (k == 0) j = l;
											}
											string f = GetStackLanguageFromSubject(Train, Subject, Section + " in " + FileName);
							...
							case "digitalgauge":
								{
									string Subject = "true";
									double LocationX = 0.0, LocationY = 0.0;
									RGBAb Color = new RGBAb(0, 0, 0, 255);
									double Radius = 0.0;
									int Layer = 0;
									double InitialAngle = -2.0943951023932, LastAngle = 2.0943951023932;
									double Minimum = 0.0, Maximum = 1000.0;
							...
							case "timetable":
								{
									...
									if (Width > 0.0 & Height > 0.0) {
										int j = CreateElement(Train, LocationX, LocationY, Width, Height, 0.5, 0.5, (double)Layer * StackDistance, PanelResolution, PanelLeft, PanelRight, PanelTop, PanelBottom, PanelBitmapWidth, PanelBitmapHeight, PanelCenterX, PanelCenterY, PanelOriginX, PanelOriginY, DriverX, DriverY, DriverZ, null, null, new RGBAb(255, 255, 255, 255), false);
		...
		private static int CreateElement(TrainManager.Train Train, double Left, double Top, double Width, double Height, double RelativeRotationCenterX, double RelativeRotationCenterY, double Distance, double PanelResolution, double PanelLeft, double PanelRight, double PanelTop, double PanelBottom, double PanelBitmapWidth, double PanelBitmapHeight, double PanelCenterX, double PanelCenterY, double PanelOriginX, double PanelOriginY, double DriverX, double DriverY, double DriverZ, Textures.Texture DaytimeTexture, Textures.Texture NighttimeTexture, RGBAb Color, bool AddStateToLastElement) {
			...
			Vertex t0 = new Vertex(v[0], new Vector2f(0.0f, 1.0f));
			Vertex t1 = new Vertex(v[1], new Vector2f(0.0f, 0.0f));
			Vertex t2 = new Vertex(v[2], new Vector2f(1.0f, 0.0f));
			Vertex t3 = new Vertex(v[3], new Vector2f(1.0f, 1.0f));
			...
			Object.Mesh.Materials[0].TransparentColor = new RGBb(0, 0, 255);

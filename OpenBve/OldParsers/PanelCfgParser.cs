		...
		internal static void ParsePanelConfig(string TrainPath, System.Text.Encoding Encoding, TrainManager.Train Train) {
			...
			// panel
			{
				if (!System.IO.File.Exists(PanelBackground)) {
					...
					Textures.Texture t;
					Textures.RegisterTexture(PanelBackground, new OpenBveApi.Textures.TextureParameters(null, RGBb.Blue), out t);
					...
					CreateElement(Train, 0, SemiHeight, w, h, FullWidth, FullHeight, WorldLeft, WorldTop, WorldWidth, WorldHeight, WorldZ + EyeDistance, Train.Cars[Train.DriverCar].DriverX, Train.Cars[Train.DriverCar].DriverY, Train.Cars[Train.DriverCar].DriverZ, t, new RGBAb(255, 255, 255, 255), false);
				}
			}
					...
					if (Lines[i].StartsWith("[", StringComparison.Ordinal) & Lines[i].EndsWith("]", StringComparison.Ordinal)) {
						...
						switch (Section.ToLowerInvariant()) {
							...
							case "圧力計":
								{
									int Type = 0;
									RGBAb[] NeedleColor = new RGBAb[] { new RGBAb(0, 0, 0, 255), new RGBAb(0, 0, 0, 255) };
										...
										int j = Lines[i].IndexOf('='); if (j >= 0) {
											...
											switch (Key.ToLowerInvariant()) {
												...
												case "上針":
													...
													NeedleColor[k] = new RGBAb((byte)r, (byte)g, (byte)b, 255);
													break;
												case "center":
												case "中心":
							...
							case "速度計":
								{
									int Type = 0;
									RGBAb Needle = new RGBAb(255, 255, 255, 255);
												...
												case "針":
													{
														...
														Needle = new RGBAb((byte)r, (byte)g, (byte)b, 255);
									...
									if (Atc != null) {
										...
										if (w > 0 & h > 0) {
											...
											for (int j = 0; j < n; j++) {
												...
												Textures.Texture t;
												Textures.RegisterTexture(Atc, new OpenBveApi.Textures.TextureParameters(new OpenBveApi.Textures.TextureClipRegion(j * h, 0, h, h), RGBb.Blue), out t);
												Textures.LoadTexture(t, Textures.OpenGlTextureWrapMode.ClampClamp);
												if (j == 0) {
													k = CreateElement(Train, x, y, (double)h, (double)h, FullWidth, FullHeight, WorldLeft, WorldTop, WorldWidth, WorldHeight, WorldZ + EyeDistance - 4.0 * StackDistance, Train.Cars[Train.DriverCar].DriverX, Train.Cars[Train.DriverCar].DriverY, Train.Cars[Train.DriverCar].DriverZ, t, new RGBAb(255, 255, 255, 255), false);
												} else {
													CreateElement(Train, x, y, (double)h, (double)h, FullWidth, FullHeight, WorldLeft, WorldTop, WorldWidth, WorldHeight, WorldZ + EyeDistance - 4.0 * StackDistance, Train.Cars[Train.DriverCar].DriverX, Train.Cars[Train.DriverCar].DriverY, Train.Cars[Train.DriverCar].DriverZ, t, new RGBAb(255, 255, 255, 255), true);
												}
											}
									...
									if (Type == 0) {
										...
										// led
										if (!NeedleOverridden) Needle = new RGBAb(0, 0, 0, 255);
										int j = CreateElement(Train, CenterX - Radius, CenterY + SemiHeight - Radius, 2.0 * Radius, 2.0 * Radius, FullWidth, FullHeight, WorldLeft, WorldTop, WorldWidth, WorldHeight, WorldZ + EyeDistance - 5.0 * StackDistance, Train.Cars[Train.DriverCar].DriverX, Train.Cars[Train.DriverCar].DriverY, Train.Cars[Train.DriverCar].DriverZ, null, Needle, false);
										double x0 = Train.Cars[Train.DriverCar].CarSections[0].Elements[j].States[0].Object.Mesh.Vertices[0].Coordinates.X;
										double y0 = Train.Cars[Train.DriverCar].CarSections[0].Elements[j].States[0].Object.Mesh.Vertices[0].Coordinates.Y;
										double z0 = Train.Cars[Train.DriverCar].CarSections[0].Elements[j].States[0].Object.Mesh.Vertices[0].Coordinates.Z;
							...
							case "digitalindicator":
								{
									...
									if (Number != null & Width > 0 & Height > 0) {
										...
										if (w > 0 & h > 0) {
											...
											for (int j = 0; j < n; j++) {
												Textures.RegisterTexture(Number, new OpenBveApi.Textures.TextureParameters(new OpenBveApi.Textures.TextureClipRegion(w - Width, j * Height, Width, Height), RGBb.Blue), out t[j]);
							...
							case "知らせ灯":
								{
									...
									if (TurnOn != null & TurnOff != null) {
										...
										Textures.RegisterTexture(TurnOn, new OpenBveApi.Textures.TextureParameters(null, RGBb.Blue), out t0);
										Textures.RegisterTexture(TurnOff, new OpenBveApi.Textures.TextureParameters(null, RGBb.Blue), out t1);
										...
										int j = CreateElement(Train, CornerX, CornerY + SemiHeight, w, h, FullWidth, FullHeight, WorldLeft, WorldTop, WorldWidth, WorldHeight, WorldZ + EyeDistance - 2.0 * StackDistance, Train.Cars[Train.DriverCar].DriverX, Train.Cars[Train.DriverCar].DriverY, Train.Cars[Train.DriverCar].DriverZ, t0, new RGBAb(255, 255, 255, 255), false);
										w = (double)t1.Width;
										h = (double)t1.Height;
										CreateElement(Train, CornerX, CornerY + SemiHeight, w, h, FullWidth, FullHeight, WorldLeft, WorldTop, WorldWidth, WorldHeight, WorldZ + EyeDistance - 2.0 * StackDistance, Train.Cars[Train.DriverCar].DriverX, Train.Cars[Train.DriverCar].DriverY, Train.Cars[Train.DriverCar].DriverZ, t1, new RGBAb(255, 255, 255, 255), true);
							...
							case "時計":
								{
									RGBAb Needle = new RGBAb(0, 0, 0, 255);
									double CenterX = 0.0, CenterY = 0.0, Radius = 16.0;
									...
									if (Image != null & Width > 0) {
										...
										if (w > 0 & h > 0) {
											...
											for (int j = 0; j < n; j++) {
												...
												Textures.RegisterTexture(Image, new OpenBveApi.Textures.TextureParameters(clip, RGBb.Blue), out t);
												...
												if (j == 0) {
													...
													CreateElement(Train, CornerX, CornerY + SemiHeight, (double)Width, (double)h, FullWidth, FullHeight, WorldLeft, WorldTop, WorldWidth, WorldHeight, WorldZ + EyeDistance - StackDistance, Train.Cars[Train.DriverCar].DriverX, Train.Cars[Train.DriverCar].DriverY, Train.Cars[Train.DriverCar].DriverZ, t, new RGBAb(255, 255, 255, 255), true);
												}
											}

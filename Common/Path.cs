...
namespace Common
{
	...
	public static class Path
	{
		...
		{
			/* this bullshit is a remnant of managed content - could be removed */
			...
			var parts = relative.Split(PathSeparationChars, StringSplitOptions.RemoveEmptyEntries);
				...
				if (parts[i].Length != 0)
				{
					...
					else
					{
						...
						else
						{
							...
							{
								var directories = System.IO.Directory.GetDirectories(absolute);
				...
				if (parts[i].Length != 0)
				{
					...
					else if (i == parts.Length - 1)
					{
						...
						else
						{
							if (System.IO.Directory.Exists(absolute))
							{
								var files = System.IO.Directory.GetFiles(absolute);
